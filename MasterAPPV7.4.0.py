...
from ui.MasterAPPV7_ui import Ui_MasterAPP
...
class MainWindow(Ui_MasterAPP):
    ...
    def setupGraphs(self, index):
        self.graphItems[index].setLabel('left', "Cable" + str(index))        
        self.graphItems[index - 1].setLabel('bottom', '')
        self.graphItems[index].setLabel('bottom', 'Time [s]')
        self.graphItems[index].getViewBox().setLimits(yMin=-0.2, yMax=2.2, xMin=-0.2, minYRange=-0.2, maxYRange=2.2, minXRange=1)
        self.graphItems[index].getViewBox().setMouseEnabled(x=True, y=False)
        self.graphItems[index].getViewBox().setRange(yRange=(0,2), xRange=(0, 10))
        self.graphItems[index].showGrid(True, False)
        self.graphItems[index].getAxis("left").setWidth(40)
        self.graphItems[index].getAxis("right").setWidth(40)
    ...
    def addCableItem(self):
        ...
        if not isStart:
            ...
            newpyqtGraph = PlotWidget(window.scrollAreaWidgetContents)
            ...
            self.graphItems[len(CableItems) - 1].setLabel('bottom', 'Time [s]')
            self.graphItems[len(CableItems) - 1].getViewBox().setLimits(yMin=-0.2, yMax=2.2, xMin=-0.2, minYRange=-0.2, maxYRange=2.2, minXRange=1)
            self.graphItems[len(CableItems) - 1].getViewBox().setMouseEnabled(x=True, y=False)
            self.graphItems[len(CableItems) - 1].getViewBox().setRange(yRange=(0,2), xRange=(0, 10))
            self.graphItems[len(CableItems) - 1].showGrid(True, False)
            self.graphItems[len(CableItems) - 1].getAxis("left").setWidth(40)
            self.graphItems[len(CableItems) - 1].getAxis("right").setWidth(40)
        else:
            isStart = False
...
class PLC_Client(): # slot has to be in undefined class
    ...
    def PLC_Listener_Start(self):
        ...
        if self.plc_stateOnly:
            ...
            LRLibrary.add_client("PLC", host=self.host, port=self.port, highestAddress=10)
            return self.plc_retry_success
        try:
    ...
    @toMainThread
    def PLC_Decypher(self, response=None):
        global CableItems
        global cableCount
        global cableUpdateProceed
        global cableUpdateInterrupt
        ...
        if action == 1:  # RecievedCableCount
            ...
            if response_item[5] == 404:
                ...
                dictionary["CS"] = str(self.getCableState(index, response_item[0]))
                window.t_label_2.setText(str(response_item[1][0]) + "°C")
                window.t_label_4.setText(str(response_item[1][1]) + "°C")
                if not self.plc_stateOnly:
                    dictionary["Position"] = str(response_item[2][0])
                    dictionary["Speed"] = str(response_item[2][1])
                    dictionary["Error"] = str(response_item[2][2])
            pass
        elif action == 4:
        ...
        cableUpdateProceed = True
        self.plc_listener_Allowed = True
    def getCableState(self, index, qinputs):
        cableState = 3
        if qinputs[0+(index*2)]:
            if qinputs[1+(index*2)]:
                cableState = 2
            else:
                cableState = 1
        else:
    ...
    def PLC_Response(self, readResponse=True):
        ...
        try:
            ...
            if not self.plc_stateOnly:
                ...
                if not int(message[3]):  #Return False if PLC has no message
                    return False
                if not readResponse:  #Check if PLC has message
                    return True
                if not not int(message[2]):  #Return False if PLC sends error message
                    return False
            ...
            plc_server.write_multiple_registers(4, [0]*9)   #clear Databank(Message read)
...
def testConnection(server):
    try:
        if server == "plc":
            ...
            sleep(1)
            testresponse = plc.PLC_Response(1)[4:7]
...
class Communication():
    ...
    def connect(self, index=-1, blank=False):
        ...
        printH(string="Připojování k " + str(self.IPList[index]) + ":" + str(self.plc_client.port()))
