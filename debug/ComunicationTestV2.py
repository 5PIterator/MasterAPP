...
class MainWindow(Ui_API_debugConsole):  # API
    ...
    def listWidgetOptions(self):
        ...
        if self.listWidget.selectedIndexes()[0].row() == 0:
            self.response = [
                ...
                cableIndex,  # index 0 - n
                tools.rnd_string(4, string.ascii_uppercase + string.digits)  # Response_item - RiderID
            ]
            '''response.extend(list(str(tools.rnd_string(4, string.digits + string.ascii_uppercase)).encode()))
            sendMessage(response)
            DataBank.set_bits(0, [1])       # Let channel 0 know there is a message
            DataBank.set_bits(2, [0])       # message for MasterAPP''' #obsolete
...
class MainWindow(Ui_PLC_debugConsole):
    def __init__(self, window) -> None:
        ...
        self.cablecount = 0
        self.cableList = cableList
        self.MasterApp_listener = threading.Thread(target=self.PLC_Listener)  # MasterApp listener thread
        ...
        self.plc_server = ModbusServer(host=socket.gethostbyname(socket.gethostname()), port=1001, no_block=True)
        pass
    ...
    def PLC_Listener(self):  # Thread only! switched on by self.MasterApp_listener
        while True:
            ...
            #if self.MasterAPP_listener_Listening and DataBank.get_words(101, 1)[0]:
    ...
    def generateCable(self, blank=False):
        ...
        if not blank:
            cableDictionary["CS"] = str(random.randrange(0, 3))
