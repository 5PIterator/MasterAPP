...
def loadSettings(valueHandler=None):
    global settingsDict
    ...
    if valueHandler is not None:
            settingsDict = valueHandler(settingsDict)
    expressValues()
...
def getValues(saveToFile:bool=True, exprSettings:bool=False, valueHandler=None):
    '''Updates the settingsDict from Qobjects. \n
    saveToFile -    If True, will automatically write into configFiles
    exprSettings -  If True, will update QtObjects with new values
    valueHandler -  If a function is provided, will call this function with 'settingsDict' argument after reading values and saves the return instead.
    settingsDict - settingsDict['sectionName']['option'][0/1 - value/QtObject]
    '''
    global settingsDict
