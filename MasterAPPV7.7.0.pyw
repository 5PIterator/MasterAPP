'''
pyinstaller --onefile --distpath G:\My' 'Drive\MasterAPPEXE --windowed MasterAPPV7.7.0.pyw
pyinstaller --onefile --distpath G:\Můj' 'Disk\MasterAPPEXE --windowed MasterAPPV7.7.0.pyw
...
def updateValues(blank=False):
    ...
    if not midnightReset.running():
        midnightReset = threadPool.submit(midnightResetCounter)
    ...
    try:
        ...
        while (cableUpdateEnabled and cableUpdateAllowed and not stopUpdate) or forceCableUpdate:
            ...
            updateInProgress = False
            print("Wait: Enter")
            while (currentTime < nextTime or saveInProgress) and not stopUpdate:
...
def saveStateDataTimer(blank: bool = False):
    ...
    while saveStateAllowed or forceSaveStateData:
        ...
        if currentTime.minute == nextMinute or forceSaveStateData:
            print("saveStateDataTimer: Enter")
            forceSaveStateData = False
...
stopMRC = False
def midnightResetCounter(blank: bool = False):
    '''Counts in the background, if current time reaches midnight, app restarts'''
    if blank: return
    printH("midnightResetCounter: Start!", 0)

    while not stopMRC:
        sleep(30)
        currentTime = datetime.now()
        if currentTime.time().hour == 23 and currentTime.time().minute == 59:
            printH("midnightResetCounter: Ready!", 0)
            while not stopMRC:
                sleep(0.5)
                currentTime = datetime.now()
                if currentTime.time().minute == 0:
                    printH("midnightResetCounter: APP Restart!", 0)
                    window.StopButton(restart=True)
                    break
midnightReset = threadPool.submit(midnightResetCounter, blank=True)
