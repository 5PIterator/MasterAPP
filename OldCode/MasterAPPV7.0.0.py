...
from ui.MasterAPPV6_ui import Ui_MasterAPP
...
class AX_Server():
    def __init__(self) -> None:
        ...
        self.request_Interval = window.ax_doubleSpinBox.value()
...
class MainWindow(Ui_MasterAPP):
    ...
    # Save Settings
    configFiles = {}
    """configFiles['FILE'][0/1 - ConfigParser()/"PATH"]"""
    settingsDict = {}
    '''settingsDict['section']['option'][0/1 - value/object]'''
    ...
    def setupSettings(self):
        ...
        self.settingsDict = {
            "masterAPP": {
                "FILE": self.configFiles['MAIN'],
                "autoStart_allowed": [autoStart_allowed, self.auto_checkBox, None],
                "autoRetry_allowed": [autoRetry_allowed, self.auto_checkBox_2, None],
                "autoUpdate_allowed": [autoUpdate_allowed, self.auto_checkBox_3, None],
                #"plc_getCableCount_allowed": [plc.plc_getCableCount_allowed, self.auto_checkBox_4, None],
                #"allowCollapse": [collapseH, self.historyCheckBox],
                "cableCount": [cablecount, self.cc_SpinBox, None],
                "retryInterval": [retryInterval, self.ri_spinBox, None],
            },

            "api": {
                "FILE": self.configFiles['MAIN'],
                "host": [api.host, self.aa_lineEdit, None],
                "port": [api.port, self.aa_lineEdit_2, None],
                "client_allowed": [api.client_allowed, self.a_checkBox, None],
                "idle_Interval": [api.idle_Interval, self.aui_doubleSpinBox, None],
            },

            "plc": {
                "FILE": self.configFiles['MAIN'],
                "host": [plc.host, self.pa_lineEdit, None],
                "port": [plc.port, self.pa_lineEdit_2, None],
                "client_allowed": [plc.client_allowed, self.p_checkBox, None],
                "autoUpdate_Interval": [autoUpdate_Interval, self.auto_doubleSpinBox, None],
            },

            "ax": {
                "FILE": self.configFiles['MAIN'],
                "host": [ax.host, self.ra_lineEdit, None],
                "port": [ax.port, self.ra_lineEdit_2, None],
                "server_allowed": [ax.server_allowed, self.ax_checkBox, None],
                "request_Interval": [ax.request_Interval, self.ax_doubleSpinBox, None],
            },

            "domain": {
                "FILE": self.configFiles['MAIN'],
                "domainHost": [domainHost, dialog.d_lineEdit_1, None],
                "domainPort": [domainPort, dialog.d_lineEdit_2, None],
                "domainUser": [domainUser, dialog.d_lineEdit_3, key],
                "domainPassword": [domainPassword, dialog.d_lineEdit_4, key],
                "saveStyle": [saveStyle, self.ss_comboBox, None],
                "localPath": [localPath, self.ss_label_2, None],
                "areal": [areal, self.d_lineEdit, None],
                "minMinuteState": [minMinuteState, self.ss_spinBox, None],
            }
        }
        pass
    def loadSettings(self):
        ...
        # Load Config
        for section in self.settingsDict.keys():
            for option in self.settingsDict[section].keys():
                if option == "FILE": continue
                widgetType = type(self.settingsDict[section][option][1])
                if widgetType == QtWidgets.QLineEdit or widgetType == QtWidgets.QLabel:
                    self.settingsDict[section][option][0] = self.settingsDict[section]['FILE'][0].get(section, option)
                if widgetType == QtWidgets.QSpinBox or widgetType == QtWidgets.QComboBox:
                    self.settingsDict[section][option][0] = self.settingsDict[section]['FILE'][0].getint(section, option)
                if widgetType == QtWidgets.QDoubleSpinBox:
                    self.settingsDict[section][option][0] = self.settingsDict[section]['FILE'][0].getfloat(section, option)
                if widgetType == QtWidgets.QCheckBox:
                    self.settingsDict[section][option][0] = self.settingsDict[section]['FILE'][0].getboolean(section, option)

                if self.settingsDict[section][option][2] is not None: # of option is encrypted, decrypt with Fernet
                    self.settingsDict[section][option][0] = Fernet(self.settingsDict[section][option][2]).decrypt(bytes(self.settingsDict[section]['FILE'][0].get(section, option) + '=', 'utf8')).decode('utf8')

                # Express settings
                if widgetType == QtWidgets.QLineEdit or widgetType == QtWidgets.QLabel:
                    self.settingsDict[section][option][1].setText(str(self.settingsDict[section][option][0]))
                elif widgetType == QtWidgets.QSpinBox or widgetType == QtWidgets.QDoubleSpinBox:
                    self.settingsDict[section][option][1].setValue(self.settingsDict[section][option][0])
                elif widgetType == QtWidgets.QComboBox:
                    self.settingsDict[section][option][1].setCurrentIndex(int(self.settingsDict[section][option][0]))
                elif widgetType == QtWidgets.QCheckBox:
                    self.settingsDict[section][option][1].setChecked(bool(self.settingsDict[section][option][0]))
        ...
        for section in self.settingsDict.keys():
            for option in self.settingsDict[section].keys():
                ...
                if widgetType == QtWidgets.QLineEdit:
                    self.settingsDict[section][option][0] = str(self.settingsDict[section][option][1].text())
                elif widgetType == QtWidgets.QSpinBox:
                    self.settingsDict[section][option][0] = int(self.settingsDict[section][option][1].value())
                elif widgetType == QtWidgets.QDoubleSpinBox:
                    self.settingsDict[section][option][0] = float(self.settingsDict[section][option][1].value())
                elif widgetType == QtWidgets.QComboBox:
                    self.settingsDict[section][option][0] = int(self.settingsDict[section][option][1].currentIndex())
                elif widgetType == QtWidgets.QCheckBox:
                    self.settingsDict[section][option][0] = bool(self.settingsDict[section][option][1].isChecked())

        # special behavior
        # ...
        self.settingsSpecialBehavior()

        # Prepare to write file
        for section in self.settingsDict.keys():
            for option in self.settingsDict[section].keys():
                if option == "FILE": continue
                if self.settingsDict[section][option][2] is not None:
                    self.settingsDict[section]['FILE'][0].set(section, option, Fernet(self.settingsDict[section][option][2]).encrypt(bytes(self.settingsDict[section][option][0], 'utf8')).decode('utf8'))
                else:
                    self.settingsDict[section]['FILE'][0].set(section, option, str(self.settingsDict[section][option][0]))

        for file in self.configFiles.keys():
            with open(self.configFiles[file][1] + '.ini', 'w') as f:
                self.configFiles[file][0].write(f)
                f.close()
    def settingsSpecialBehavior(self):
        global stopClients
        ...
        if self.settingsDict["masterAPP"]["autoUpdate_allowed"][0]:
        ...
        if self.settingsDict["api"]["client_allowed"][0] != self.a_checkBox.isChecked():
            if not (api.api_retry_success or plc.plc_retry_success):
                self.settingsDict["api"]["client_allowed"][0] = self.a_checkBox.isChecked()
            elif self.a_checkBox.isChecked():
                start = True
            else:
                stop = True

        if self.settingsDict["plc"]["client_allowed"][0] != self.p_checkBox.isChecked():
            if not (api.api_retry_success or plc.plc_retry_success):
                self.settingsDict["plc"]["client_allowed"][0] = self.p_checkBox.isChecked()
        ...
        if not (
                self.settingsDict["api"]["client_allowed"][0] or
                self.settingsDict["plc"]["client_allowed"][0] or
                self.settingsDict["ax"]["server_allowed"][0]
                ):
            self.retry_pushButton.setEnabled(False)
        ...
        #Check Adresses
        if not self.settingsDict["domain"]["areal"][0] == '':
            self.settingsDict["domain"]["areal"][0] += '/'
        pass

        restart = False
        if self.settingsDict["api"]["host"][0] != self.aa_lineEdit.text():  # API Address(host)
            if self.aa_lineEdit.text() != '':
                self.settingsDict["api"]["host"][0] = self.aa_lineEdit.text()
                restart = True
            else:
                self.settingsDict["api"]["host"][0] = socket.gethostbyname(socket.gethostname())
                self.aa_lineEdit.setText('')
        if str(self.settingsDict["api"]["port"][0]) != self.aa_lineEdit_2.text():  # API Address(port)
            if self.aa_lineEdit_2.text() != '' and self.aa_lineEdit_2.text().isdigit():
                self.settingsDict["api"]["port"][0] = int(self.aa_lineEdit_2.text())
                restart = True
            else:
                self.settingsDict["api"]["port"][0] = 1000
                self.aa_lineEdit_2.setText('')
        if self.settingsDict["plc"]["host"][0] != self.pa_lineEdit.text():  # PLC Address(host)
            if self.pa_lineEdit.text() != '':
                self.settingsDict["plc"]["host"][0] = self.pa_lineEdit.text()
                restart = True
            else:
                self.settingsDict["plc"]["host"][0] = socket.gethostbyname(socket.gethostname())
                self.pa_lineEdit.setText('')
        if str(self.settingsDict["plc"]["port"][0]) != self.pa_lineEdit_2.text():  # PLC Address(port)
            if self.pa_lineEdit_2.text() != '' and self.pa_lineEdit_2.text().isdigit():
                self.settingsDict["plc"]["port"][0] = int(self.pa_lineEdit_2.text())
                restart = True
            else:
                self.settingsDict["plc"]["port"][0] = 502
                self.pa_lineEdit_2.setText('')
        if str(self.settingsDict["ax"]["host"][0]) != self.ra_lineEdit.text():  # AX Address(host)
            if self.ra_lineEdit.text() != '' and self.ra_lineEdit.text().isdigit():
                self.settingsDict["ax"]["host"][0] = int(self.ra_lineEdit.text())
                restart = True
            else:
                self.settingsDict["ax"]["host"][0] = "192.168.11.50"
                self.ra_lineEdit.setText('')
        if str(self.settingsDict["ax"]["port"][0]) != self.ra_lineEdit_2.text():  # AX Address(port)
            if self.ra_lineEdit_2.text() != '' and self.ra_lineEdit_2.text().isdigit():
                self.settingsDict["ax"]["port"][0] = int(self.ra_lineEdit_2.text())
                restart = True
            else:
                self.settingsDict["ax"]["port"][0] = 11854
                self.ra_lineEdit_2.setText('')

        autoStart_allowed = self.settingsDict["masterAPP"]["autoStart_allowed"][0]
        autoRetry_allowed = self.settingsDict["masterAPP"]["autoRetry_allowed"][0]
        autoUpdate_allowed = self.settingsDict["masterAPP"]["autoUpdate_allowed"][0]
        #plc_getCableCount_allowed = self.settingsDict["masterAPP"]["plc_getCableCount_allowed"][0]
        cableCount = self.settingsDict["masterAPP"]["cableCount"][0]
        retryInterval = self.settingsDict["masterAPP"]["retryInterval"][0]

        api.host = self.settingsDict["api"]["host"][0]
        api.port = self.settingsDict["api"]["port"][0]
        api.client_allowed = self.settingsDict["api"]["client_allowed"][0]
        api.idle_Interval = self.settingsDict["api"]["idle_Interval"][0]

        plc.host = self.settingsDict["plc"]["host"][0]
        plc.port = self.settingsDict["plc"]["port"][0]
        plc.client_allowed = self.settingsDict["plc"]["client_allowed"][0]
        autoUpdate_Interval = self.settingsDict["plc"]["autoUpdate_Interval"][0]

        ax.host = self.settingsDict["ax"]["host"][0]
        ax.port = self.settingsDict["ax"]["port"][0]
        ax.server_allowed = self.settingsDict["ax"]["server_allowed"][0]
        ax.request_Interval = self.settingsDict["ax"]["request_Interval"][0]

        domainHost = self.settingsDict["domain"]["domainHost"][0]
        domainPort = self.settingsDict["domain"]["domainPort"][0]
        domainUser = self.settingsDict["domain"]["domainUser"][0]
        domainPassword = self.settingsDict["domain"]["domainPassword"][0]
        saveStyle = self.settingsDict["domain"]["saveStyle"][0]
        localPath = self.settingsDict["domain"]["localPath"][0]
        areal = self.settingsDict["domain"]["areal"][0]
        minMinuteState = self.settingsDict["domain"]["minMinuteState"][0]
        ...
        if (restart or start) and (api.api_retry_success or plc.plc_retry_success):
            ...
            if messagebox.askokcancel("Confirm Action", "Variables of either API or PLC have been changed during runtime. \nThis change can only be implemented after restart. \n\nDo you wish to proceed?"):
                ...
                self.aa_lineEdit.setText(self.settingsDict["api"]["host"][0])
                self.aa_lineEdit_2.setText(str(self.settingsDict["api"]["port"][0]))
                self.pa_lineEdit.setText(self.settingsDict["plc"]["host"][0])
                self.pa_lineEdit_2.setText(str(self.settingsDict["plc"]["port"][0]))
                self.a_checkBox.setChecked(self.settingsDict["api"]["client_allowed"][0])
                self.p_checkBox.setChecked(self.settingsDict["plc"]["client_allowed"][0])
        elif stop:
            root = tkinter.Tk()
            root.withdraw()
            if messagebox.askokcancel("Confirm Action", "Variables of either API or PLC have been changed during runtime. \nServer will now stop. \n\nDo you wish to proceed?"):
                ...
                self.p_checkBox.setChecked(self.settingsDict["plc"]["client_allowed"][0])
        pass
    def checkConfigFile(self, overwrite = True):
        #check file
        for file in self.configFiles.keys():
            try:
                self.configFiles[file][0].read(self.configFiles[file][1] + '.ini')
            except:
                printH("An error occurred while loading settings.")
                if not overwrite: return False

        for section in self.settingsDict.keys():
            if not self.settingsDict[section]['FILE'][0].has_section(section):
                printH("A config file '" + self.settingsDict[section]['FILE'][1] + "' is missing section '" + section + "'")

                if not overwrite: return False
                self.settingsDict[section]['FILE'][0].add_section(section)
                for option in self.settingsDict[section].keys():
                    if option == "FILE": continue
                    self.settingsDict[section]['FILE'][0].set(section, option, str(self.settingsDict[section][option][0])),
                continue
            ...
            for option in self.settingsDict[section].keys():
                if option == "FILE": continue
                if not self.settingsDict[section]['FILE'][0].has_option(section, option):
                    printH("A config file '" + self.settingsDict[section]['FILE'][1] + "' is missing option '" + option + "' in section '" + section)
    ...
    def loadSettings(self):
        ...
        self.aa_lineEdit.setPlaceholderText(api.host + " (This Machine)")
        self.aa_lineEdit_2.setPlaceholderText(str(api.port))
        self.pa_lineEdit.setPlaceholderText(plc.host + " (This Machine)")
        self.pa_lineEdit_2.setPlaceholderText(str(plc.port))
        self.ra_lineEdit.setPlaceholderText(ax.host)
        self.ra_lineEdit_2.setPlaceholderText(str(ax.port))
        ...
        api.idle_Interval = config.getfloat('api', 'interval')
        api.client_allowed = config.getboolean('api', 'allowed')
        api.host = config.get('api', 'host')
        api.port = config.getint('api', 'port')
        autoUpdate_Interval = config.getfloat('plc', 'interval')
        plc.client_allowed = config.getboolean('plc', 'allowed')
        plc.host = config.get('plc', 'host')
        plc.port = config.getint('plc', 'port')
        plc.plc_getCableCount_allowed = config.getboolean('auto', 'allowGetCount')
        cablecount = config.getint('auto', 'cableCount')
        ax.request_Interval = config.getfloat('ax', 'interval')
        ax.server_allowed = config.getboolean('ax', 'allowed')
        ax.host = config.get('ax', 'host')
        ax.port = config.getint('ax', 'port')
        ...
        # Express settings
        checkSettings = False
        self.aa_lineEdit.setText(api.host)
        self.aa_lineEdit_2.setText(str(api.port))
        self.pa_lineEdit.setText(plc.host)
        self.pa_lineEdit_2.setText(str(plc.port))
        self.ra_lineEdit.setText(ax.host)
        self.ra_lineEdit_2.setText(str(ax.port))
        self.ri_spinBox.setValue(int(retryInterval))
        self.aui_doubleSpinBox.setValue(api.idle_Interval)
        self.auto_doubleSpinBox.setValue(autoUpdate_Interval)
        self.ax_doubleSpinBox.setValue(ax.request_Interval)
        ...
        self.a_checkBox.setChecked(api.client_allowed)
        self.p_checkBox.setChecked(plc.client_allowed)
        self.ax_checkBox.setChecked(ax.server_allowed)
    ...
    def updateSettings(self):
        ...
        api.idle_Interval = self.aui_doubleSpinBox.value()
        ...
        ax.request_Interval = self.ax_doubleSpinBox.value()
        ax.server_allowed = self.ax_checkBox.isChecked()
        ...
        stop = False
        start = False
        if api.client_allowed != self.a_checkBox.isChecked():
            if not (api.api_retry_success or plc.plc_retry_success):
                api.client_allowed = self.a_checkBox.isChecked()
            elif self.a_checkBox.isChecked():
                start = True
            else:
                stop = True

        if plc.client_allowed != self.p_checkBox.isChecked():
            if not (api.api_retry_success or plc.plc_retry_success):
                plc.client_allowed = self.p_checkBox.isChecked()
            elif self.p_checkBox.isChecked():
                start = True
            else:
                stop = True
        ...
        restart = False
        if api.host != self.aa_lineEdit.text():  # API Address(host)
            if self.aa_lineEdit.text() != '':
                api.host = self.aa_lineEdit.text()
                restart = True
            else:
                api.host = socket.gethostbyname(socket.gethostname())
                self.aa_lineEdit.setText('')
        if str(api.port) != self.aa_lineEdit_2.text():  # API Address(port)
            if self.aa_lineEdit_2.text() != '' and self.aa_lineEdit_2.text().isdigit():
                api.port = int(self.aa_lineEdit_2.text())
                restart = True
            else:
                api.port = 1000
                self.aa_lineEdit_2.setText('')
        if plc.host != self.pa_lineEdit.text():  # PLC Address(host)
            if self.pa_lineEdit.text() != '':
                plc.host = self.pa_lineEdit.text()
                restart = True
            else:
                plc.host = socket.gethostbyname(socket.gethostname())
                self.pa_lineEdit.setText('')
        if str(plc.port) != self.pa_lineEdit_2.text():  # PLC Address(port)
            if self.pa_lineEdit_2.text() != '' and self.pa_lineEdit_2.text().isdigit():
                plc.port = int(self.pa_lineEdit_2.text())
                restart = True
            else:
                plc.port = 502
                self.pa_lineEdit_2.setText('')
        if str(ax.host) != self.ra_lineEdit.text():  # AX Address(host)
            if self.ra_lineEdit.text() != '' and self.ra_lineEdit.text().isdigit():
                ax.host = int(self.ra_lineEdit.text())
                restart = True
            else:
                ax.host = "192.168.11.50"
                self.ra_lineEdit.setText('')
        if str(ax.port) != self.ra_lineEdit_2.text():  # AX Address(port)
            if self.ra_lineEdit_2.text() != '' and self.ra_lineEdit_2.text().isdigit():
                ax.port = int(self.ra_lineEdit_2.text())
                restart = True
            else:
                ax.port = 11854
                self.ra_lineEdit_2.setText('')
        ...
        config.set('api', 'interval', str(api.idle_Interval))
        config.set('api', 'allowed', str(self.a_checkBox.isChecked()))
        config.set('api', 'host', str(api.host))
        config.set('api', 'port', str(api.port))
        config.set('plc', 'interval', str(autoUpdate_Interval))
        config.set('plc', 'allowed', str(self.p_checkBox.isChecked()))
        config.set('plc', 'host', str(plc.host))
        config.set('plc', 'port', str(plc.port))
        config.set('ax', 'interval', str(ax.request_Interval))
        config.set('ax', 'allowed', str(self.ax_checkBox.isChecked()))
        config.set('ax', 'host', str(ax.host))
        config.set('ax', 'port', str(ax.port))
