    ...
    def API_Listener_Start(self): #deactivated
        ...
        try:
            ...
            printH(
                ...
                "No API instance detected on the other side\n" +
                "of this port.",
                4, str(err))
            return self.api_retry_success
...
class PLC_Client(): # slot has to be in undefined class
    ...
    def PLC_Listener_Start(self):
        ...
        try:  # Port test
            ...
            printH(
                "An error occurred while connecting to\n" +
                str(self.plc_host) + ":" + str(self.plc_port) + "\n" +
                "Most likely case is that port on this\n" +
                "address is either not binded or in use\n" +
                "by another instance of MasterAPP or\n" +
                "other application.",
        ...
        if self.plc_stateOnly:
            printH(
                ...
                "be attempted to be retrieved.", 4, str(self.plc_host) + ":" + str(self.plc_port) + " <---> Quido <-X-> PLC")
        ...
        try:
            ...
            printH(
                ...
                "No compatible PLC instance detected on the other side\n" +
                "of this port",
                4, err)
            return self.plc_retry_success
    ...
    def PLC_Listener(self):  # Thread only! switched on by retry
        ...
        while True:
            if stopClients or not self.plc_client_allowed:
                printH("Connection with " + str(self.plc_host) + " interrupted\nPLC-Listener Offline", 0, "MasterAPP <-X-> PLC", isRootEnd=True)
                plc_server.close()
                self.plc_retry_success = False
                decypher.stop()
                break
            ...
            if not plc_server.is_open():
                ...
                if not testConnection("plc"):
                    ...
                    printH("Connection lost with " + str(self.plc_host) + " \nPLC-Listener Offline", 0, "MasterAPP <-X-> PLC", isRootEnd=True)
                    self.plc_listener_Allowed = False
                    self.plc_retry_success = False
                    if autoRetry_allowed:
        ...
        if action == "GetCableCount" or action == "1":
            ...
            if self.plc_stateOnly:
                ...
                if response is False:
                    printH(
                        "Failed to get response from Quido - " + str(i) + "/5" + "\n" +
...
class MainWindow(Ui_MasterAPP):
    ...
    autoMove = True
    lastData = None
    def updateGraph(self, loadFile = True):
        ...
        if loadFile:
            ...
            if len(os.listdir(wPath)) == 0:
                while len(graphData) < len(CableItems):
                    graphData.append([[],[]])
                return
            for filedir in os.listdir(wPath):
                with open(wPath + filedir, 'r') as file:
                    ...
                    for line in lines:
                        ...
                        graphTime: list = graphData[int(index)][0] #X
        ...
        for graphitem in graphItems:
            ...
            graphitem.clear()
            graphitem.plot(graphData[index][0], graphData[index][1])
            index += 1
    ...
    def SaveStateDataButton(self):
        ...
        if not cableUpdate.running:
            threadPool.submit(saveStateData, True, False)
    ...
    def StopButton(self, restart = False):
        ...
        printH("Clients/Servers Stopped", 0)
    ...
    def clearH(self, ask=False):
        ...
        for x in range(1, self.historyList.topLevelItemCount()):
            ...
            self.historyList.invisibleRootItem().removeChild(
                self.historyList.invisibleRootItem().child(i))
...
def Retry(loop = False):
    ...
    if ((not api.api_client_allowed or api.api_retry_success) and
        ...
        if api.api_host == plc.plc_host and api.api_client_allowed and plc.plc_client_allowed:
            printH(
                "API/PLC Connection successfully \nestabilished with:\n" +
    ...
    while (cableUpdateEnabled and cableUpdateAllowed) or forceCableUpdate:
        try:
            ...
            if time.time() - start_time > autoUpdate_Interval:
                printH("MasterAPP cannot keep up! \nReducing update rate by 1s.", 1, str(time.time() - start_time)[:5] + " > " + str(autoUpdate_Interval))
                window.auto_doubleSpinBox.setValue(window.auto_doubleSpinBox.value() + 1)
                autoUpdate_Interval = window.auto_doubleSpinBox.value()
            forceCableUpdate = False
            ...
            while currentTime < nextTime:
                ...
                window.auto_label_1.setText(str(nextTime.second - datetime.datetime.now().time().second).zfill(2))
                time.sleep(0.01)
                currentTime = datetime.datetime.now().time()
...
logFilePath = None
hItemCount = 0
def printH(string: str, level=100, description="", isRootEnd=False):
    global parentitem
    global parentTree
    global hTuple
    global hItemCount
    ...
    '''print(" "*level + string +"/ "+ description)
    return returnItem'''
    ...
    try:
        with open(logFilePath, 'a') as logfile:
            logfile.write(" "*level + string + " / " + description + "\n")
            logfile.close()
    except:
        pass
...
def saveStateData():
    ...
    if (cableUpdateEnabled and cableUpdateAllowed) or forceSaveStateData == True:
        forceSaveStateData = False
        if not cableUpdateEnabled: return
        saveInProgress = True
    ...
    try:
        ...
        printH(
            "Process Failed!\nCouldn't send " + dataPath, 1, description=str(err))
    else:
        printH("Success!", 2)
    saveInProgress = False
    return
