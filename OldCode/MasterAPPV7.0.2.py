...
from ToMainThreadV1 import FuncToMain, tmt, addSignalList
import SettingsManagerV1 as SM
from SettingsManagerV1 import settingsDict as SD

class MainWindow(Ui_MasterAPP):
    resized = QtCore.pyqtSignal()
    p1 = QtCore.pyqtSignal()
    p2 = QtCore.pyqtSignal()
    p3 = QtCore.pyqtSignal()
    p4 = QtCore.pyqtSignal()
    p5 = QtCore.pyqtSignal(int)
    p6 = QtCore.pyqtSignal(bool)
    ...
    def __init__(self, window) -> None:
        ...
        # EventHandlers
        addSignalList("api_decypher", self.p1).connect(api.API_Decypher)
        addSignalList("plc_decypher", self.p2).connect(plc.PLC_Decypher)
        addSignalList("addCableItem", self.p3).connect(self.addCableItem)
        addSignalList("updateGraphs", self.p4).connect(self.updateGraph2)
        addSignalList("setupGraphs", self.p5).connect(self.setupGraphs)
        addSignalList("clearH", self.p6).connect(self.clearH)
        ...
        self.signalList["api_decypher"].connect(api.API_Decypher)
    ...
    def SaveSettings(self):
        ...
        cableUpdateAllowed = self.cb_auto_cableUpdate.isEnabled()
        plc.plc_getCableCount_allowed = self.cb_auto_cableCount.isChecked()
    ...
    def setupGraphs(self, index):
        if not isinstance(threading.current_thread(), threading._MainThread):
            FuncToMain(self.setupGraphs, index)
            return
...
class API_Client():
    ...
    def API_Decypher(self, handler: AXHandler = None):
        if not isinstance(threading.current_thread(), threading._MainThread):
            FuncToMain(self.API_Decypher, handler)
            return
...
def update_CableItems(blank: bool = False):
        ...
        try:
            ...
            '''if currentTime.minute == nextMinute:
                ...
                if backupInterval < autoUpdate_Interval:
                    ...
                    window.dsb_plcInterval.setValue(window.dsb_plcInterval.value() - 1)
            ...
            if not cableUpdateEnabled:
                ...
                window.auto_label_1.setText(str(0).zfill(2))
                window.dsb_plcInterval.setEnabled(True)
    ...
    window.dsb_plcInterval.setValue(backupInterval)
