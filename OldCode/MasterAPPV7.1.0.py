...
import sys, threading, datetime, socket, os, tkinter, queue
import ftplib
...
from ToMainThreadV2 import TMT, toMainThread
...
def update_CableItems(blank: bool = False):
    ...
    while datetime.datetime.now().time().second != 0 and not ssdt.running() and False: #synchronize to a minute
        if forceCableUpdate or not cableUpdateEnabled: break
        window.updateLabelCounter(window.auto_label_1, str(60 - datetime.datetime.now().time().second).zfill(2), 'minute sync')
        sleep(1)
...
def saveStateDataTimer(blank: bool = False):
    ...
    while saveStateAllowed or forceSaveStateData:
        ...
        window.updateLabelCounter(window.ss_label_1, str(60 - datetime.datetime.now().time().second).zfill(2))
