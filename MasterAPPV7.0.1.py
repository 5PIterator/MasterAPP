...
import sys, threading, datetime, socket, os, tkinter
import ftplib
        ...
        if self.index == 0:
            ...
            while True:
                if api.api_recieved:
                    ...
                    sleep(0.01) # Wait for the signal to process
...
class MainWindow(Ui_MasterAPP):
    ...
    def __init__(self, window) -> None:
        ...
        self.ri_spinBox.valueChanged.connect(self.SaveSettings)
        self.aui_doubleSpinBox.valueChanged.connect(self.SaveSettings)
        self.auto_doubleSpinBox.valueChanged.connect(self.SaveSettings)
        self.ax_doubleSpinBox.valueChanged.connect(self.SaveSettings)
        self.cc_SpinBox.valueChanged.connect(self.SaveSettings)
        self.aa_lineEdit.editingFinished.connect(self.SaveSettings)
        self.aa_lineEdit_2.editingFinished.connect(self.SaveSettings)
        self.pa_lineEdit.editingFinished.connect(self.SaveSettings)
        self.pa_lineEdit_2.editingFinished.connect(self.SaveSettings)
        self.ra_lineEdit.editingFinished.connect(self.SaveSettings)
        self.ra_lineEdit_2.editingFinished.connect(self.SaveSettings)
        self.a_checkBox.stateChanged.connect(self.SaveSettings)
        self.p_checkBox.stateChanged.connect(self.SaveSettings)
        self.ax_checkBox.stateChanged.connect(self.SaveSettings)
        self.auto_checkBox.stateChanged.connect(self.SaveSettings)
        self.auto_checkBox_2.stateChanged.connect(self.SaveSettings)
        self.auto_checkBox_3.stateChanged.connect(self.SaveSettings)
        self.auto_checkBox_4.stateChanged.connect(self.SaveSettings)
        self.ss_comboBox.currentIndexChanged.connect(self.SaveSettings)
        self.ss_spinBox.valueChanged.connect(self.SaveSettings)
        self.d_lineEdit.editingFinished.connect(self.SaveSettings)
    ...
    def setupSettings(self):
        ...
        SM.addOption('masterAPP', 'autoStart_allowed', autoStart_allowed, self.auto_checkBox)
        SM.addOption('masterAPP', 'autoRetry_allowed', autoRetry_allowed, self.auto_checkBox_2)
        SM.addOption('masterAPP', 'autoUpdate_allowed', autoUpdate_allowed, self.auto_checkBox_3)
        SM.addOption('masterAPP', 'cableCount', cablecount, self.cc_SpinBox)
        SM.addOption('masterAPP', 'retryInterval', retryInterval, self.ri_spinBox)

        SM.addSection('MAIN', 'api')
        SM.addOption('api', 'host', api.host, self.aa_lineEdit)
        SM.addOption('api', 'port', api.port, self.aa_lineEdit_2)
        SM.addOption('api', 'client_allowed', api.client_allowed, self.a_checkBox)
        SM.addOption('api', 'idle_Interval', api.idle_Interval, self.aui_doubleSpinBox)

        SM.addSection('MAIN', 'plc')
        SM.addOption('plc', 'host', plc.host, self.pa_lineEdit)
        SM.addOption('plc', 'port', plc.port, self.pa_lineEdit_2)
        SM.addOption('plc', 'client_allowed', plc.client_allowed, self.p_checkBox)
        SM.addOption('plc', 'autoUpdate_Interval', autoUpdate_Interval, self.auto_doubleSpinBox)

        SM.addSection('MAIN', 'ax')
        SM.addOption('ax', 'host', ax.host, self.ra_lineEdit)
        SM.addOption('ax', 'port', ax.port, self.ra_lineEdit_2)
        SM.addOption('ax', 'server_allowed', ax.server_allowed, self.ax_checkBox)
        SM.addOption('ax', 'request_Interval', ax.request_Interval, self.ax_doubleSpinBox)
        ...
        SM.addOption('domain', 'saveStyle', saveStyle, self.ss_comboBox)
        SM.addOption('domain', 'localPath', localPath, self.ss_label_2)
        SM.addOption('domain', 'areal', areal, self.d_lineEdit)
        SM.addOption('domain', 'minMinuteState', minMinuteState, self.ss_spinBox)
    ...
    def SaveSettings(self):
        ...
        if SM._expressing: return
        SM.getValues(False)
        stop = False
        start = False

        if SD["masterAPP"]["autoUpdate_allowed"][0]:
        ...
        if SD["api"]["client_allowed"][0] != api.client_allowed:
            if not (api.api_retry_success or plc.plc_retry_success):
                SD["api"]["client_allowed"][0] = api.client_allowed
            elif api.client_allowed:
                start = True
            else:
                stop = True

        if SD["plc"]["client_allowed"][0] != plc.client_allowed:
            if not (api.api_retry_success or plc.plc_retry_success):
        ...
        if not (
                SD["api"]["client_allowed"][0] or
                SD["plc"]["client_allowed"][0] or
                SD["ax"]["server_allowed"][0]
                ):
            self.retry_pushButton.setEnabled(False)
        ...
        #Check Adresses
        if not SD["domain"]["areal"][0] == '':
            SD["domain"]["areal"][0] += '/'
        pass

        restart = False
        if SD["api"]["host"][0] != api.host:  # API Address(host)
            if api.host != '':
                SD["api"]["host"][0] = api.host
                restart = True
            else:
                SD["api"]["host"][0] = socket.gethostbyname(socket.gethostname())
                self.aa_lineEdit.setText('')
        if str(SD["api"]["port"][0]) != api.port:  # API Address(port)
            if api.port != '' and isinstance(api.port, int):
                SD["api"]["port"][0] = int(api.port)
                restart = True
            else:
                SD["api"]["port"][0] = 1000
                self.aa_lineEdit_2.setText('')
        if SD["plc"]["host"][0] != plc.host:  # PLC Address(host)
            if plc.host != '':
                SD["plc"]["host"][0] = plc.host
                restart = True
            else:
                SD["plc"]["host"][0] = socket.gethostbyname(socket.gethostname())
                self.pa_lineEdit.setText('')
        if str(SD["plc"]["port"][0]) != plc.port:  # PLC Address(port)
            if plc.port != '' and isinstance(plc.port, int):
                SD["plc"]["port"][0] = int(plc.port)
                restart = True
            else:
                SD["plc"]["port"][0] = 502
                self.pa_lineEdit_2.setText('')
        if str(SD["ax"]["host"][0]) != ax.host:  # AX Address(host)
            if ax.host != '' and ax.host.isdigit():
                SD["ax"]["host"][0] = int(ax.host)
                restart = True
            else:
                SD["ax"]["host"][0] = "192.168.11.50"
                self.ra_lineEdit.setText('')
        if str(SD["ax"]["port"][0]) != ax.port:  # AX Address(port)
            if ax.port != '' and isinstance(ax.port, int):
                SD["ax"]["port"][0] = int(ax.port)
                restart = True
            else:
                SD["ax"]["port"][0] = 11854
        ...
        #plc_getCableCount_allowed = SD["masterAPP"]["plc_getCableCount_allowed"][0]
        cableCount = SD["masterAPP"]["cableCount"][0]
        retryInterval = SD["masterAPP"]["retryInterval"][0]

        api.host = SD["api"]["host"][0]
        api.port = SD["api"]["port"][0]
        api.client_allowed = SD["api"]["client_allowed"][0]
        api.idle_Interval = SD["api"]["idle_Interval"][0]

        plc.host = SD["plc"]["host"][0]
        plc.port = SD["plc"]["port"][0]
        plc.client_allowed = SD["plc"]["client_allowed"][0]
        autoUpdate_Interval = SD["plc"]["autoUpdate_Interval"][0]

        ax.host = SD["ax"]["host"][0]
        ax.port = SD["ax"]["port"][0]
        ...
        if (restart or start) and (api.api_retry_success or plc.plc_retry_success):
            ...
            if messagebox.askokcancel("Confirm Action", "Variables of either API or PLC have been changed during runtime. \nThis change can only be implemented after restart. \n\nDo you wish to proceed?"):
                ...
                self.aa_lineEdit.setText(SD["api"]["host"][0])
                self.aa_lineEdit_2.setText(str(SD["api"]["port"][0]))
                self.pa_lineEdit.setText(SD["plc"]["host"][0])
                self.pa_lineEdit_2.setText(str(SD["plc"]["port"][0]))
                self.a_checkBox.setChecked(SD["api"]["client_allowed"][0])
                self.p_checkBox.setChecked(SD["plc"]["client_allowed"][0])
        elif stop:
            root = tkinter.Tk()
            root.withdraw()
    ...
    def updateGraph2(self):
        if self.graphData == []:
            return
        if FuncToMain(self.updateGraph2): return
        ...
        try:
            if cablecount == len(CableItems):
                ...
                while c < cablecount:
                    sleep(.1)
                    if cablecount > len(CableItems):
                        self.add_CableItem() # add Cable in the main thread
                    self.setupGraphs()
    ...
    def setupGraphs(self, index):
        if FuncToMain(self.setupGraphs): return
    ...
    def add_CableItem(self):
        if FuncToMain(self.add_CableItem): return
    ...
    def clearH(self, ask=True):
        if FuncToMain(self.clearH): return
...
class API_Client():
    ...
    def API_Decypher(self, handler: AXHandler = None):
        if FuncToMain(self.API_Decypher): return
...
class PLC_Client(): # slot has to be in undefined class
    ...
    def PLC_Decypher(self):
        if FuncToMain(self.PLC_Decypher): return
        ...
        if action == 1:  # RecievedCableCount
            ...
            #sleep(0.1)  # wait for RecievedCableCount to process
            forceCableUpdate = True
            self.plc_recieved = False
            self.plc_listener_Allowed = True
            '''if not cableUpdate.running():
                ...
                for i in range(0, cablecount):  # Does support double digit cables!
                    ...
                    if api.api_retry_success:
                        ...
                        if cableUpdateInterrupt:
                            ...
                            aF += 1
            if cableUpdateInterrupt:
                printH("Cable update interrupted! (" + str(time() - start_time)[:5] + "s)", 0,
                        "api:" + str(aF) + "/" + str(cablecount) + "(" + str(aE) + "E), " +
                        "plc:" + str(pF) + "/" + str(cablecount) + "(" + str(pE) + "E)",
                        isRootEnd=True)
                break
            else:
                printH("Cable update complete! (" + str(time() - start_time)[:5] + "s)", 0,
                        "api:" + str(aF) + "/" + str(cablecount) + "(" + str(aE) + "E), " +
                        "plc:" + str(pF) + "/" + str(cablecount) + "(" + str(pE) + "E)",
                        isRootEnd=True)
            if stopRetry:
