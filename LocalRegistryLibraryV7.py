...
def WriteLibrary(client:LRLClient, fromUpdate=False):
    ...
    while t < timeout:
        try:
            for reg in client._newRegister:
...
def getRegisters(client:str, regType:str, address:int, amount:int=None):
    ...
    DIIN - DiscreteInput
    COIL - Coils'''
    if regType != "HORE" and regType != "COIL" and regType != "INRE" and regType != "DIIN":
    ...
    busy = True
    localRegister = {
        "HORE": clientList[client]._localHoldingRegisters,
        "INRE": clientList[client]._localInputRegisters,
        "DIIN": clientList[client]._localDiscreteInputs,
        "COIL": clientList[client]._localCoils,
    }
    ...
    if amount is None:
        """if debug:
            ...
            if readServer:
                ...
                registers = clientList[client]._localRegisters[address]
        ...
        if regType == "HORE" or regType == "INRE":
            registers = numpy.int16(registers)
...
def setRegister(client:str, regType:str, address:int, value, wordIndex:int=None, force=False):
    ...
    HORE - HoldingRegister
    COIL - Coils'''
    if regType != "HORE" and regType != "COIL":
        raise Exception("setRegister(): '"+ str(regType) + "' is not a valid register type")
    global busy
    ...
    newRegister = {
        "HORE": clientList[client]._newRegister,
        "COIL": clientList[client]._newCoil
    }
    writtenReg = None
    register = None
    ...
    if writtenReg is None: # if grabbed from last read, create new register to send
        newRegister.append((address, register))
    else: # if grabbed from written reg, overwrite
        newRegister[writtenReg] = (address, register)
...
def setRegisters(client:str, regType:str, address:int, registers:list):
    ...
    newRegister = {
        "HORE": clientList[client]._newRegisterSet,
        "COIL": clientList[client]._newCoilSet
    }
    if regType == "HORE":
